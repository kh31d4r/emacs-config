...
*** Quality of Life
    ...
    Enable =auto-revert-mode= when viewing documents and images, to
    automatically refresh them in case they change externally or get
    re-generated.
...
** Navigation
   ...
   Use =ibuffer=, bound to =C-x C-b= to organize buffers, by type or by project.
   #+BEGIN_SRC emacs-lisp
     (defun setup-ibuffer-filters ()
     ...
     (use-package ibuffer-projectile
       ...
       (add-hook 'ibuffer-hook #'setup-ibuffer-filters)
       (setup-ibuffer-filters))
...
** Eglot
   ...
   Stop eglot from writing nonsense to the minibuffer.
   #+BEGIN_SRC emacs-lisp
...
*** Lua
    ...
    #+END_SRC
*** Python
    Use =eglot= to provide IDE features like auto complete and
    goto-definition. Requires a language server to be installed, like
    [[https://github.com/python-lsp/python-lsp-server][pylsp]] or [[https://github.com/emacs-lsp/lsp-pyright][pyright]].
    #+BEGIN_SRC emacs-lisp
      (use-package eglot
...
** GTD
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun capture-target-path (target-name)
       (expand-file-name
        (concat target-name ".org")
        (car org-agenda-files)))

     (setq org-capture-templates
           `(("l" "life")
             ("ll" "Todo" entry (file ,(capture-target-path "life"))
              (file ,(capture-template-path "todo")))
             ("lc" "Todo with checklist" entry (file ,(capture-target-path "life"))
              (file ,(capture-template-path "todo-checklist")))
             ("lo" "Org-mode Todo" entry (file+headline ,(capture-target-path "life") "Org-mode things")
              (file ,(capture-template-path "todo-org" "life")))
             ("lb" "Book" entry (file+headline ,(capture-target-path "~/org/life.org") "Books")
              (file ,(capture-template-path "books" "life")))
             ("lw" "Wishlist Item" entry (file+headline ,(capture-target-path "life") "Wishlist")
              (file ,(capture-template-path "wishlist-item" "life")))
             ("lj" "Journal Entry" entry (file+datetree ,(capture-target-path "journal"))
              (file ,(capture-template-path "journal" "life")))
             ("w" "work")
             ("ww" "Todo" entry (file ,(capture-target-path "work"))
              (file ,(capture-template-path "todo")))
             ("wc" "Todo with checklist" entry (file ,(capture-target-path "work"))
              (file ,(capture-template-path "todo-checklist")))
             ("q" "quote" entry (file ,(capture-target-path "quotes"))
